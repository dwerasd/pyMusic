...
class MainWindow(QMainWindow):
    ...
    def restart_app(self) -> None:
        """현재 인스턴스를 종료하고 새 인스턴스를 실행한다."""
        self._save_settings()
        self._save_playlist_to_disk()
        self.engine.stop()
        if self.tray is not None:
            self.tray.hide()
        # 단일 인스턴스 서버를 닫아 새 프로세스가 이름을 점유할 수 있게 한다.
        bridge = getattr(self, "_bridge", None)
        if bridge is not None and bridge.server is not None:
            bridge.server.close()
        # 절대 경로 + --restart 플래그로 재실행 (새 프로세스가 이전 서버 해제를 대기).
        if getattr(sys, "frozen", False):
            cmd = [sys.executable, "--restart"]
        else:
            cmd = [sys.executable, os.path.abspath(sys.argv[0]), "--restart"]
    ...
    def on_play_clicked(self) -> None:
        ...
        if row < 0:
            ...
            if self.tracks:
                checked = self.controller.checked_indices()
                row = checked[0] if checked else 0
            else:
                return
...
def main() -> int:
    ...
    if is_restart:
        # 이전 인스턴스가 서버를 해제할 때까지 대기 (최대 ~5초).
        import time
        for _ in range(50):
            if not bridge.send_to_existing_instance({"cmd": "ping"}):
                break
            time.sleep(0.1)
    elif bridge.send_to_existing_instance({
